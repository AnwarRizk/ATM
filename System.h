    ...
    if(!sz(requests)){
        ...
        switch(opt)
        {
            ...
            case 2:
              ...
              memset(requests[i].reason, '\0', sizeof(requests[i].reason));
              strcpy(requests[i].reason, "refused");
...
void add_admin(){
    ...
    set_color(h, 15);
    char pass[8];
    cout << "\t\tEnter admin password [4 characters]: ";
    set_password(pass, 4);
    if(length(pass) != 4){
        set_color(h, 12);
